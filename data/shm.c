...
uint64_t shm_alloc(uint64_t size_bytes, uint64_t* out_vaddr) {
    ...
    for (uint64_t i = 0; i < page_count; i++) {
        hal_map_page_in_space((uint64_t)current_thread->owner->page_directory, my_vaddr + (i * 4096), obj->phys_pages[i], 0x207);
    }
...
uint64_t shm_map(uint64_t shm_id) {
    ...
    for (uint64_t i = 0; i < obj->page_count; i++) {
        hal_map_page_in_space((uint64_t)current_thread->owner->page_directory, target_vaddr + (i * 4096), obj->phys_pages[i], 0x207);
    }
